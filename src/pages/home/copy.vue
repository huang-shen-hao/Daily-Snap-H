          ...
          <view class="swiper-slider">
            ...
            <scroll-view class="scroll-task" scroll-y="true">
              <view v-for="sub in card.sub_tasks" :key="sub.id" class="sub-task">
                <checkbox-group @change="onSubChange(sub.documentId, sub.isComplete, card.id)">
...
const onSubChange = async (id: string, status: boolean, tid: number) => {
  const res = await changeSubTaskStatus(id, !status, tid)
  if (res.code === 0) {
    await getTasks()
    // uni.showToast({
    //   title: res.message,

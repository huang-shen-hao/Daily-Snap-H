  ...
  <view class="wrapper">
    ...
    <view class="editor-wrapper">
      <editor
        ...
        @input="whenInput"
      ></editor>
      <view style="text-align: center; position: fixed; bottom: 60rpx; left: 50%; transform: translateX(-50%)">
        <button size="medium" @click="clickBtnSubmit">保存</button>
      </view>
    </view>
  </view>
</template>
...
// 根据键盘高度调整工具栏 bottom
const adjustToolbar = (kbHeight: number) => {
  console.log('>> 调整工具栏 > 键盘高度', kbHeight)
  toolbarBottom.value = kbHeight
}

onMounted(() => {
  uni.onKeyboardHeightChange(({ height }) => {
    adjustToolbar(height)
  })
})
onBeforeUnmount(() => {
  uni.offKeyboardHeightChange(adjustToolbar)
})
</script>
